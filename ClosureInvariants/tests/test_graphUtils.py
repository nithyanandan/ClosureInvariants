...
def test_generate_independent_triads_valid(example_ids):
    baseid = 1
    triads = GU.generate_independent_triads(example_ids, baseid)
...
def test_generate_independent_triads_valid_strings(example_ids_strings):
    baseid = 'A'
    triads = GU.generate_independent_triads(example_ids_strings, baseid)
...
def test_generate_independent_triads_invalid_ids():
    with pytest.raises(TypeError):
        GU.generate_independent_triads(123, 1)

def test_generate_independent_triads_invalid_baseid(example_ids):
    with pytest.raises(TypeError):
        GU.generate_independent_triads(example_ids, 1.5)

def test_generate_independent_triads_invalid_baseid_value(example_ids):
    with pytest.raises(ValueError):
        GU.generate_independent_triads(example_ids, 5)

def test_generate_independent_triads_invalid_ids_length():
    with pytest.raises(ValueError):
        GU.generate_independent_triads([1], 1)
